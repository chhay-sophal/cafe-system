...
async function confirmDelete() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : t("recipe.deleteError");
  } finally {
...
<template>
  <div v-if="open && product" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
    <div class="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
      <h2 class="text-lg font-bold text-slate-900">{{ $t("recipe.deleteHeading") }}</h2>
      <p class="mt-2 text-sm text-slate-600">
        <i18n-t keypath="recipe.deleteConfirm" tag="span">
          <template #name><span class="font-semibold">{{ product.name }}</span></template>
          <template #count>{{ ingredientCount }}</template>
        </i18n-t>
      </p>
      ...
      <div class="mt-6 flex justify-end gap-3">
        ...
        <button
          ...
          @click="confirmDelete"
        >
          {{ isDeleting ? $t("common.deleting") : $t("recipe.deleteHeading") }}
        </button>
      </div>
    </div>
  </div>
